...
public class Cycle {
  ...
  public Iterable<Integer> cycle() {
   ...
   private void dfs(Graph G, int nodoDaCuiProvengo, int nodoSuCuiMiTrovo) {
       marked[nodoSuCuiMiTrovo] = true;
       for (int figlioDelNodo : G.adj(nodoSuCuiMiTrovo)) {

           // short circuit if cycle already found
           if (cycle != null) return;

           if (!marked[figlioDelNodo]) {
               edgeTo[figlioDelNodo] = nodoSuCuiMiTrovo;
               dfs(G, nodoSuCuiMiTrovo, figlioDelNodo);
           }

           // check for cycle (but disregard reverse of edge leading to v)
           else if (figlioDelNodo != nodoDaCuiProvengo) {
               cycle = new Stack<Integer>();
               for (int x = nodoSuCuiMiTrovo; x != figlioDelNodo; x = edgeTo[x]) {
                   cycle.push(x);
               }
               cycle.push(figlioDelNodo);
               cycle.push(nodoSuCuiMiTrovo);
           }
       }
   }
   ...
   public static void main(String[] args) {
       In in = new In(new File("test_tinyG.txt"));
