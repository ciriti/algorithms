...
public class BreadthFirstPaths {
    ...
    public static void main(String[] args) {
        ...
        int verticeDaAnalizzare = Integer.parseInt("0");
        BreadthFirstPaths bfs = new BreadthFirstPaths(G, verticeDaAnalizzare);

        for (int altroVertice = 0; altroVertice < G.V(); altroVertice++) {
            if (bfs.hasPathTo(altroVertice)) {
                StdOut.printf("%d to %d (%d):  ", verticeDaAnalizzare, altroVertice, bfs.distTo(altroVertice));
                for (int x : bfs.pathTo(altroVertice)) {
                    if (x == verticeDaAnalizzare) 
                    	StdOut.print("-" + x);
                    else        
                    	StdOut.print("-" + x);
                }
                ...
                StdOut.printf("%d to %d (-):  not connected\n", verticeDaAnalizzare, altroVertice);
            }
