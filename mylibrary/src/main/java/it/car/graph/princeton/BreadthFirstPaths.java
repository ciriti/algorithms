package it.car.graph.princeton;
...
import static it.car.graph.princeton.GraphGenerator.complete;
...
public class BreadthFirstPaths {
    ...
    private void bfs(Graph G, Iterable<Integer> sources) {
        Queue<Integer> q = new LinkedList<>();//new Queue<Integer>();
        for (int s : sources) {
            marked[s] = true;
            distTo[s] = 0;
            q.add(s);//enqueue(s);
...
//        In in = new In(args[0]);
        ...
        System.out.println("Insert number of vertex:");
        int V = scanner.nextInt();//Integer.parseInt(args[0]);
//        System.out.println("Insert number of edge:");
//        int E = scanner.nextInt();//Integer.parseInt(args[1]);
        scanner.close();
        Graph G;
//        G = GraphGenerator.regular(V, 14);//new Graph(in);
        G = GraphGenerator.cycle(V);//new Graph(in);
        // StdOut.println(G);

        int s = new Random().nextInt(G.V());//Integer.parseInt(args[1]);
